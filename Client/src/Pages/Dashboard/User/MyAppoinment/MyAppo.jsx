...
const MyAppo = () => {
  ...
  const [paymentStatuses, setPaymentStatuses] = useState({}); // To store payment statuses
  ...
  useEffect(() => {
    const fetchPaymentInfo = async () => {
      try {
        const response = await axiosSecure.get('/paymentInfo');
        const statuses = response.data.reduce((acc, payment) => {
          acc[payment.paymentId] = payment.status; // Use paymentId if _id is not matching
          return acc;
        }, {});
    ...
    try {
      ...
      if (response.status === 200) {
        setAppointments((prev) =>
          prev.filter((appointment) => appointment._id !== appointmentToDelete)
        );
        setIsOpen(false);
              ...
              <tr key={appointment._id} className="border-t">
                ...
                <td className="py-2 px-4">
                  <button
                    ...
                    disabled={paymentStatuses[appointment._id] === "Success"}
                  >
                    Pay
                  </button>
                  <button
